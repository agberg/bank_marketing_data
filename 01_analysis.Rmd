...
eda <- all_data %>%
  ...
  summarize(count = n(),
            percentage = n()/head(total, 1))

eda

```

- Over time (Macro variable, month)
- % of Cell Phone
- Age binned
...
```{r}
set.seed(3456)
trainIndex <- createDataPartition(all_data$term_deposit, p = .5, 
                                  list = FALSE, 
                                  times = 1)
...
caret_cv <- train(term_deposit ~ ., method = 'ranger', data = training_data_to_use, trControl = trainctrl, metric = 'Kappa', tuneLength = 1, importance = 'impurity')
...
saveRDS(ranger_model, "ranger_model.RDS")
saveRDS(caret_cv, "caret_cv.RDS")
...
```
How do I add the expected value per category?
